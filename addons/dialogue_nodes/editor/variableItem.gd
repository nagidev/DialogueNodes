...
func _ready():
	...
	for i in range(type.get_item_count()):
		type.set_item_id(i, types[i])
	
	type.connect("item_selected", self, "_on_type_changed")
	
	var_name.connect("text_changed", self, "_on_modified")
	type.connect("item_selected", self, "_on_modified")
	string_value.connect("text_changed", self, "_on_modified")
	int_value.connect("value_changed", self, "_on_modified")
	float_value.connect("value_changed", self, "_on_modified")
	bool_value.connect("toggled", self, "_on_modified")
...
func set_value(new_value):
	match types[type.selected]:
		...
		TYPE_BOOL:
			bool_value.pressed = bool(new_value)
